    ...
    while power > -1:
        ...
        for i, poly in enumerate(before):
            if poly.power == power and poly.x == True:
                if poly.sign == '-':
                    x_factor = x_factor + (poly.factor * -1)
                else:
                    x_factor = x_factor + poly.factor
        if x_factor > 0:
            sign = '+'
        else:
            sign = '-'
...
def vb_display(before, phase):
    ...
    result = result + "= 0"
    if phase == 1:
        print("First step of reducted equation: ", result)
    if phase == 2:
        print("Second step of reducted equation: ", result)
    if phase == 3:
        print("Third step of reducted equation: ", result)
...
def get_real(before, final):
    ...
    if real < 0:
        tmp = real * -1
    before.append(Polynom(sign, tmp, 0, False, sign + str(tmp) ))
    ...
    real = round(real, 2)
    if real != 0:
        return Polynom(sign, real, 0, False, sign + str(real) )
...
def reduce(before, after, vb, debug):
    ...
    final.append(get_real(before, final))
    if vb is True:
        vb_display(before, 2)
    if vb is True:
        vb_display(final, 3)
